...
# print(post(BASE_URL+RECIPIENT_URL, json={"_id":"112", "name": "Testing Post Recipient"}).text)
# 2. Retrieve recipient
# print(get(BASE_URL+RECIPIENT_URL, json={"_id": "15"}).text) 
# 3. Update recipient
# print(put(BASE_URL+RECIPIENT_URL+'/112', json={'$set': {'name': 'NOT az ASPARAGUS'}}).text) 
# 4. Delete recipient
# print(delete(BASE_URL+RECIPIENT_URL+'/112').text) 
...
# 1. Add a new item
print(post(BASE_URL+ITEMS_URL, json={"_id":"117", "name": "Testing Post item", "type":"food(chinese)"}).text)
print(post(BASE_URL+ITEMS_URL, json={"_id":"113", "name": "Testing Post item", "type":"food(chinese)"}).text)
print(post(BASE_URL+ITEMS_URL, json={"_id":"2733", "name": "Testing Post item", "type":"food(chinese)"}).text)
...
# 3. Update item
# print(put(BASE_URL+ITEMS_URL, json={'query': {"type":"food(chinese)"}, 'update': {'$set': {'name': 'Noodles'}}}).text) 
# 4. Delete item
3# print(delete(BASE_URL+ITEMS_URL, json={"type":"food(chinese)"}).text) 